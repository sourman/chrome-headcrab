// chrome-headcrab driver — the daemon spawned by `chrome-headcrab attach`.
//
...
//
// Lifecycle:
//   attach → chrome-headcrab spawns `node driver.mjs <wsUrl> <socketPath>`
//   eval   → bash opens the socket, sends {eval:"<js>"}, reads one response
//   detach → bash kills the driver PID (Chrome and profile stay up)
...
function makeEvalTimeout(initialMs) {
  ...
  const timeoutErrMsg = `eval body timed out after ${bodyTimeoutMs}ms — common causes: ` +
    ...
    'a stuck network/loader, or a runaway loop. If wedged, run chrome-headcrab detach + attach to recover.';
...
server.listen(socketPath, () => {
  // signal readiness on stdout (chrome-headcrab attach waits for this)
