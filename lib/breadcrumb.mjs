...
export const BREADCRUMB_DIR =
  process.env.HC_BREADCRUMB_DIR || `${homedir()}/.cache/chrome-headcrab/breadcrumbs`;
...
function resolvePath(idOrLabel) {
  ...
  const matches = files
    .map((f) => {
      ...
      return typeof rec.label === 'string' && rec.label.toLowerCase().includes(String(idOrLabel).toLowerCase()) ? { full, rec } : null;
    })
    .filter(Boolean);
...
export function createBreadcrumb({ cdp, evalInPage }) {
  ...
  function buildRecord({ stopping = false } = {}) {
    return {
      id: recording.id,
      ...
      status: stopping ? 'stopped' : 'recording',
    };
  }
  ...
  async function stop() {
    if (!recording) throw new Error('breadcrumb: not recording');
    const record = buildRecord({ stopping: true });
    mkdirSync(BREADCRUMB_DIR, { recursive: true });
    const path = filePathFor(record.id);
    writeFileSync(path, JSON.stringify(record, null, 2));
    if (unsubFrame) {
  ...
  async function replay(idOrLabel) {
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      if (ev.type === 'navigate' && ev.url) {
        ...
        manualSteps.push({ index: i, type: 'action', action: ev.action, detail: ev.detail });
        stepsSkipped++;
      } else {
        stepsSkipped++;
      }
